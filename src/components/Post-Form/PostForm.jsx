...
function PostForm({ post }) {
    ...
    const submit = async(data) => {
        if(post){
          const file = data.image[0] ? await appwriteService.getFilePreview(data.image[0]) : null
  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="flex flex-wrap">
        ...
        <div className="w-1/3 px-2">
            ...
            />
            <Button className="w-full" type="submit" bgColor={post ? "bg-green-600" : undefined}>{post ? "Update" : "Submit"}</Button>
        </div>
    </form>
  )
}
